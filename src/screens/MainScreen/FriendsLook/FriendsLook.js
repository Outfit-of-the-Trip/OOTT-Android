...
const FriendsLook = ({route}) => {
  ...
  const handleNameClick = name => {
    //팝업창에서 친구 선택되었을떄 처리 method
    setfriendname(name);
    setModalVisible(false);
  };

  const renderItem = ({item}) => {
    //DB에서 친구 목록 불러오는 Method
  ...
  return (
    <SafeAreaView style={styles.allconatiner}>
      <ImageBackground
      ...
      >
        <View
          ...
          <View style={styles.topshowcontainer}>
            ...
            />
            <Text style={styles.toptext}>양준민{travlFreinds}</Text>
        ...
        <View
          ...
          <Text style={styles.bottomtext2}>#cute #japan_look</Text>
        </View>
      </ImageBackground>
    </SafeAreaView>
  );
